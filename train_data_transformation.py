    ...
    def foo(self):
        ...
        if True:
            ...
            for i in range(15):
                print t, i
                if t == 0:
                    tmp_train_x = [train_x[i][h] for h in range(1, N)]
                    tmp_train_y = [train_y[i][h] for h in range(1, N)]
                elif t == N - 1:
                    tmp_train_x = [train_x[i][h] for h in range(N - 1)]
                    tmp_train_y = [train_y[i][h] for h in range(N - 1)]
                else:
                    tmp_train_x = [train_x[i][h] for h in range(t)] + [train_x[i][h] for h in range(t + 1, N)]
                    tmp_train_y = [train_y[i][h] for h in range(t)] + [train_y[i][h] for h in range(t + 1, N)]

                p = prediction(tmp_train_x, tmp_train_y, [np.ma.concatenate([train_x[i][t]])])
                print train_idd[t]
                if not ("ainl" in train_data[affect][train_idd[t]]):
                    train_data[affect][train_idd[t]]["ainl"] = []
                train_data[affect][train_idd[t]]["ainl"].append(p[0])
...
for YYY in range(1):
 ...
 for affect in affect_list:
  ...
  for train_part in [train_data]:
    ...
    f = open(os.path.join(cwd, 'dataset', 'task1', 'train_ainl', 'dataset_json_' + affect + '.txt'), 'w+')
    f.write(a)
    f.close()
